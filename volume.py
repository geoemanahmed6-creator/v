...
# ========== الشريط الجانبي (الإعدادات العامة) ==========
with st.sidebar:
...
# ========== إذا كانت البيانات موجودة، نعرض الرسوم والإحصائيات ==========
if st.session_state.data_stored:
    ...
    # تصدير PDF للرسوم البيانية جميعها
    def export_to_pdf():
        # قائمة بالأشكال
        figs = [fig1, fig2, fig3, fig4, fig5, fig6]
        # تحويل كل شكل إلى صورة PNG في الذاكرة
        images = []
        for i, f in enumerate(figs):
            img_bytes = pio.to_image(f, format='png', width=1000, height=600, scale=2)
            img = Image.open(io.BytesIO(img_bytes))
            images.append(img)
        # دمج الصور في ملف PDF واحد باستخدام img2pdf
        pdf_bytes = img2pdf.convert([img for img in images])
        return pdf_bytes

    if st.button("📑 Export All Charts as PDF", use_container_width=True):
        with st.spinner("Generating PDF report..."):
            pdf_data = export_to_pdf()
            st.download_button("✅ Download PDF Report", pdf_data, "charts_report.pdf", "application/pdf")
else:
